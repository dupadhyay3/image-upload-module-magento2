    ...
    public function execute()
    {
        ...
        if ($imageId) {
            try {
                ...
                $resultRedirect->setPath('adminimageupload/image/index');
                return $resultRedirect;
            } catch (NoSuchEntityException $e) {
                $this->messageManager->addErrorMessage(__('The image no longer exists.'));
                return $resultRedirect->setPath('adminimageupload/image/index');
            } catch (LocalizedException $e) {
                $this->messageManager->addErrorMessage($e->getMessage());
                return $resultRedirect->setPath('adminimageupload/image/index', ['image_id' => $imageId]);
            } catch (\Exception $e) {
                $this->messageManager->addErrorMessage(__('There was a problem deleting the image'));
                return $resultRedirect->setPath('adminimageupload/image/edit', ['image_id' => $imageId]);
            }
        }
